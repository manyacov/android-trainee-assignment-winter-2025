...
@Composable
fun LocalAudioPlayerScreen(
    modifier: Modifier,
    viewModel: LocalAudioPlayerViewModel
) {
    ...
    AudioPlayerScreen(
        ...
        onPrevious = { currentTrackId ->
            viewModel.setEvent(
                AudioPlayerContract.Event.OnPreviousClicked(currentTrackId)
            )
        },
        onNext = { currentTrackId ->
            viewModel.setEvent(
                AudioPlayerContract.Event.OnNextClicked(currentTrackId)
            )
        }
    )
}